...
import Prelude                hiding ((++), (!!), length, map, repeat, tail,
                                      unzip3, zip3, zipWith)
...
import CLaSH.Signal           (Clock,Reset,Signal,(.&&.), not1, regEn, unbundle)
 ...
 * @dom@ is the domain to which the circuit is synchronised.
...
-- "CLaSH.Prelude.Mealy"
mealyDF :: (?clk :: Clock clk dom, ?res :: Reset res dom)
        => (s -> i -> (s,o))
        -> s
        -> DataFlow dom Bool Bool i o
...
-- "CLaSH.Prelude.Moore"
mooreDF :: (?clk :: Clock clk dom, ?res :: Reset res dom)
        => (s -> i -> s)
        -> (s -> o)
        -> s
        -> DataFlow dom Bool Bool i o
...
-- @
fifoDF :: forall dom res clk addrSize m n a .
...
--
-- <<doc/loopDF_sync.svg>>
loopDF :: (KnownNat m, KnownNat n, KnownNat addrSize,
          (m+n) ~ (2^addrSize), ?res :: Reset res dom, ?clk :: Clock clk dom)
       => SNat (m + n) -- ^ Depth of the FIFO buffer. Must be a power of two
